	...
	struct tm *res)
{
  ...
  /* compute day of week */
  if ((weekday = (int)((ADJUSTED_EPOCH_WDAY + days) % DAYSPERWEEK)) < 0)
    weekday += DAYSPERWEEK;
  res->tm_wday = weekday;
  ...
  era = (int)((days >= 0 ? days : days - (DAYS_PER_ERA - 1)) / DAYS_PER_ERA);
  eraday = days - era * DAYS_PER_ERA;	/* [0, 146096] */
  erayear = (unsigned)((eraday - eraday / (DAYS_PER_4_YEARS - 1) + eraday / DAYS_PER_CENTURY -
      eraday / (DAYS_PER_ERA - 1)) / 365);	/* [0, 399] */
  yearday = (unsigned)(eraday - (DAYS_PER_YEAR * erayear + erayear / 4 - erayear / 100));	/* [0, 365] */
